...
struct usb_type{
  ...
  unsigned char tx_ep1[12];                                                     // 23
  volatile unsigned char len_ep1;                                               // 35
  unsigned char rx_buffer[250];                                                 // 36
...
void usb_send_answer_ep1(){
  PC_ODR = (PC_ODR & 0x3F) | 0x40;
  PC_DDR |= 0xC0;
  PC_CR1 |= 0xC0;

  usb_tx_count = usb.len_ep1;
  usb_tx_buffer_pointer = &usb.tx_ep1[0];
...
unsigned char USB_SendPacket(unsigned char *buffer, unsigned char length){
  ...
  while(length){
    ...
    if(length >= 8){
      ...
      usb.tx_ep1[5] = buffer[3]; crc = table[(crc ^ buffer[3]) & 0xFF] ^ (crc >> 8);
      usb.tx_ep1[6] = buffer[4]; crc = table[(crc ^ buffer[4]) & 0xFF] ^ (crc >> 8);
      usb.tx_ep1[7] = buffer[5]; crc = table[(crc ^ buffer[5]) & 0xFF] ^ (crc >> 8);
      usb.tx_ep1[8] = buffer[6]; crc = table[(crc ^ buffer[6]) & 0xFF] ^ (crc >> 8);
      usb.tx_ep1[9] = buffer[7]; crc = table[(crc ^ buffer[7]) & 0xFF] ^ (crc >> 8);
      crc = ~crc;
      usb.tx_ep1[10] = crc;
      usb.tx_ep1[11] = crc >> 8;
      buffer += 8;
      ...
      for(index = 2; index < (2 + length); index++){
        crc = table[(crc ^ *buffer) & 0xFF] ^ (crc >> 8);
        usb.tx_ep1[index] = *buffer++;
      }
      crc = ~crc;
      usb.tx_ep1[index++] = crc;
      usb.tx_ep1[index] = crc >> 8;
